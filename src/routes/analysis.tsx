...
function AnalysisPage() {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      setMapUrl(url);
      const noWaste = loadNoWasteDetected();
      if (noWaste) {
        setStatus("empty");
  ...
  return (
    ...
    >
      ...
      <main className="grid flex-1 grid-cols-[360px_1fr] min-h-0">
        <aside id="dashboard" className="border-r border-border bg-card/95 backdrop-blur p-4 shadow-2xl overflow-y-auto">
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-md border border-border bg-background/40 p-3">
              <Button onClick={runAnalysis} disabled={status === "running" || !usingGeneratedMap || status === "empty"} className="w-full">
                {status === "running" ? (
                  <>
